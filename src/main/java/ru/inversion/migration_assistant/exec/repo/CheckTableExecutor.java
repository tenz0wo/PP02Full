    ...
    void defineQueries () {
        if (dbType == DBType.ORACLE) {
            ...
            queryNotEmpty = "SELECT LEAST(COUNT(*),1)\n" +
                            "  FROM DUAL\n" +
                            " WHERE EXISTS (SELECT NULL FROM " + params.getSchema() + "." + params.getTable() + ")";
        } else if (dbType == DBType.POSTGRES) {
            queryExists = "SELECT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = '" + params.getSchema() + "' AND tablename = '" + params.getTable() + "') AS table_exists";
            queryNotEmpty = "SELECT EXISTS (SELECT 1 FROM " + params.getSchema() + "." + params.getTable() + " LIMIT 1) AS table_not_empty";
        } else {
